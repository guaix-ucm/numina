...
def extract_slice(input, axis, i1, i2, method, wavecal, transpose, vmin, vmax, noplot, output, png=None):
    ...
    if not noplot:
        ...
        fig, ax = plt.subplots()
        img = ax.imshow(
            slice2d, origin="lower", extent=extent, vmin=vmin, vmax=vmax, aspect=aspect, interpolation="None"
        )
...
def main(args=None):
    ...
    parser.add_argument(
        "--method", help="Collapse method (default=sum)", type=str, default="sum", choices=["sum", "mean", "median"]
    )
    parser.add_argument(
        "--wavecal",
        help="Wavelength calibration type",
        type=str,
        default="none",
