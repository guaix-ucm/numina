...
def generate_mosaic_of_2d_images(
    ...
    # generate mosaic
    logger.info(f"Reprojection method: {reproject_method}")
    for i in range(nimages):
        ...
        if reproject_method == "interp":
            image2d_resampled, footprint_image2d_resampled = reproject_interp(
                input_data=hdu2d_image, output_projection=wcs_mosaic2d, shape_out=shape_mosaic2d
            )
            if hdu2d_mask is not None:
                mask2d_resampled, _ = reproject_interp(
                    input_data=hdu2d_mask, output_projection=wcs_mosaic2d, shape_out=shape_mosaic2d
                )
            else:
        ...
        if mask2d_resampled is None:
            ...
            stack3d[i, :, :].mask = np.logical_or(
                np.logical_or(mask2d_resampled, mask2d_resampled == np.nan), (footprint_image2d_resampled == 0)
            )
    ...
    if combination_function == "mean":
        ...
        array2d_sigclip_mean, array2d_sigclip_median, array2d_sigclip_stddev = sigma_clipped_stats(
            stack3d, axis=0, maxiters=None
        )
    ...
    logger.info(f"Combination function: {combination_function}")
...
def main(args=None):
    ...
    parser.add_argument("input_list", help="TXT file with list of 2D images to be combined", type=str)
    ...
    parser.add_argument(
        ...
        help="Combination function. Default: mean",
        type=str,
        default="mean",
        ...
        if len(fname) > 0:
            if fname[0] not in ["#"]:
                ...
                with fits.open(fname) as hdul:
                    ...
                    logger.info(f"{hdu2d_image.header['NAXIS1']=}")
                    logger.info(f"{hdu2d_image.header['NAXIS2']=}")
                    if extname_mask is not None:
                        if extname_mask in hdul:
                            hdu2d_mask = hdul[extname_mask].copy()
                            logger.info(f"image mask: {extname_mask}")
                    ...
                    if hdu2d_mask is None:
                        hdu2d_mask = fits.ImageHDU(data=np.isnan(hdu2d_image.data).astype(np.uint8))
                        hdu2d_mask.header.extend(wcs_to_header_using_cd_keywords(WCS(hdu2d_image.header)), update=True)
                        logger.info("generating mask from np.nan values")
                    logger.info(f"Number of masked pixels / total: {np.sum(hdu2d_mask.data)} / {hdu2d_mask.size}")
