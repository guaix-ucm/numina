...
def measure_slice_xy_offsets_in_3d_cube(
    ...
    if npoints_along_naxis3 > naxis3:
        logger.warning(f"WARNING: {npoints_along_naxis3=} > {naxis3=}")
        logger.warning("         forcing npoints_along_naxis3 = naxis3")
        npoints_along_naxis3 = naxis3

    # reference slice for the first iteration
    if islice_reference is None:
        islice_reference = naxis3 // 2
        logger.warning(f"WARNING: setting {islice_reference=}")
    ...
    for iteration in range(niterations):
        ...
        logger.info(f"iteration: {iteration} --> reference slice in [{i1_ref+1}:[{i2_ref}] (FITS convention)")
        if plots:
            fig, ax = plt.subplots()
        ...
        for islice in islice_array:
            ...
            if method == 1:
                yx_offsets, _, _ = phase_cross_correlation(
                    reference_image=slice_reference, moving_image=slice_binned, upsample_factor=100, overlap_ratio=0.90
                )
            elif method == 2:
                yx_offsets = yx_offsets_correlate2d(reference_image=slice_reference, moving_image=slice_binned)
            else:
        ...
        for yfit, title in zip([delta_x_slice, delta_y_slice], ["delta_x_slice", "delta_y_slice"]):
            if plots:
                debugplot = 1
                fig = plt.figure()
            else:
...
def main(args=None):
    ...
    parser = argparse.ArgumentParser(
        description="Determine (X,Y) offsets between slices along NAXIS3", formatter_class=RichHelpFormatter
    )
    ...
    parser.add_argument(
        "--extname", help="Output extension name to store result (default None)", type=str, default="None"
    )
    parser.add_argument("--polydeg", help="Polynomial degree to fit distortion", type=int, default=2)
    parser.add_argument(
        "--times-sigma-reject", help="Times sigma to reject fitted offsets (default 3.0)", type=float, default=3.0
    )
    parser.add_argument(
        "--islice-reference", help="Initial pixel corresponding to the reference slice (from 1 to NAXIS3)", type=int
    )
    ...
    if extname != "NONE":
        logger.info(f"Updating file {args.filename}")
