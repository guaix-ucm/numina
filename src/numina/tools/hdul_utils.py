...
def get_hdu_from_hdul(hdul, extname=None, extnum=None):
    ...
    # Check if the specified extension exists in the HDUList
    if extname_image not in hdul:
        logger.error(f"Expected '{extname_image}' extension not found")
        sys.exit(1)
...
def get_wcs_from_hdul(hdul, extname=None, extnum=None, wcskey=None):
    """Get WCS from HDUList.

    The HDU can be specified either by extension name or extension number.
    If both are provided, an error is raised. If neither is provided,
    the first extension (0) is used by default.

    Once the extension is determined, the function checks for multiple
    WCS in the header. If multiple WCS are found, the user must specify
    which one to use via the `wcskey` parameter. If only one WCS is found,
    it is used directly.
    ...
    # Get the HDU from the HDUList using the provided extension name or number
