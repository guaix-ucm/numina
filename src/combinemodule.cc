    ...
    PyObject *args, PyObject *kwds)
{
  ...
  int ok = PyArg_ParseTupleAndKeywords(args, kwds,
      "OO!O!O!O!O!O!O!O!O!O!:internal_combine_with_offsets", kwlist, &method,
      &PyList_Type, &images, &PyList_Type, &masks, &PyArray_Type, &offsets,
      &PyArray_Type, &out[0], &PyArray_Type, &out[1], &PyArray_Type, &out[2],
      &PyTuple_Type, &margs, &PyArray_Type, &zeros, &PyArray_Type, &scales,
    ...
    PyObject *kwds)
{
  ...
  static char *kwlist[] = { "method", "reject", "data", "masks", "out0", "out1", "out2",
      "args", "zeros", "scales", "weights", NULL };

  int ok = PyArg_ParseTupleAndKeywords(args, kwds,
      "ssO!O!O!O!O!O!O!O!O!:internal_combine", kwlist, &method, &reject,
      &PyList_Type, &images, &PyList_Type, &masks, &PyArray_Type, &out[0], &PyArray_Type,
      &out[1], &PyArray_Type, &out[2], &PyTuple_Type, &margs, &PyArray_Type,
      &zeros, &PyArray_Type, &scales, &PyArray_Type, &weights);
  ...
  // Method class
  std::auto_ptr<Numina::CombineMethod> method_ptr;
  // Reject class
  std::auto_ptr<Numina::RejectMethod> reject_ptr;

  try
  {
    // A factory class for named functions
    method_ptr.reset(Numina::CombineMethodFactory::create(method, margs));
  } catch (MethodException& ex)
  {
      // If there is a problem during construction
      return PyErr_Format(CombineError,
  ...
  {
    // A factory class for named functions
    reject_ptr.reset(Numina::RejectMethodFactory::create(reject, margs));
  } catch (MethodException& ex)
  {
      // If there is a problem during construction
      return PyErr_Format(CombineError,
