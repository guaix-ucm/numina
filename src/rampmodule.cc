...
static PyObject* py_loopover(PyObject *self, PyObject *args, PyObject *kwds)
{
  ...
  PyObject* ret = NULL; // A five tuple: (out, var, nmap, mask, crmask)
  ...
  char *kwlist[] = {"inpt", "dt", "gain", "ron", "badpixels", "out", "var",
  ...
  if(!PyArg_ParseTupleAndKeywords(args, kwds, "&Oddd|O&O&Oidi:loopover_ramp_c",
        kwlist, &PyArray_Converter, &inp,
        &dt, &gain, &ron,
        &badpixels,
        &PyArray_OutputConverter, &out,
        &PyArray_OutputConverter, &var,
  ...
  /*
   * FIXME
   * Here we have to consider only input, output and variance
   * as all the other arrays are dtype uint8
   */
  arrs[0] = inp;
  if(out != NULL)
   valid = 2;
  ...
  dtypes[1] = PyArray_DescrFromType(NPY_USHORT); // badpix
  dtypes[2] = PyArray_DescrFromType(common->type); // value
  dtypes[3] = PyArray_DescrFromType(common->type); // variance
  dtypes[4] = PyArray_DescrFromType(NPY_USHORT); // number of pixels
  dtypes[5] = PyArray_DescrFromType(NPY_USHORT); // new mask of bad pixels
  dtypes[6] = PyArray_DescrFromType(NPY_USHORT); // new mask of cosmic rays
